...
def main(mvt):
    ...
    plt = DataPlanchePlotting(dfi, restrict_to_joints=["acromioclavicular"])
    plt.plot()
    plt.update_style()
    plt.show()

    if export:
        plt.fig.write_image(f"../../plots/{mvt}_AC.png")
        plt.fig.write_image(f"../../plots/{mvt}_AC.pdf")
        plt.fig.write_html(f"../../plots/{mvt}_AC.html", include_mathjax="cdn")
